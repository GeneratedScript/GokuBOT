...
client.on("guildMemberAdd", member => {
  ...
  member.user.addRole("name", "Member");
});
...
client.on('message', message => {
 ...
 if (command == "ssj") {
  message.channel.sendMessage("Super... SAIYAN!!!", {
...
if (command === "cusannounce") {
  ...
  if (command == "eval") {
    if (message.author.id !== "223557159151992832") return;
    try {
      var code = args.join(" ");
...
function clean(text) {
  if (typeof (text) === "string")
    ...
    return text;
            ...
            client.login('MzgwMzkzNTYwNjc3MDIzNzQ0.DO38oQ.MFTgTThEPV4vYDIzM_djUoBfnuw');